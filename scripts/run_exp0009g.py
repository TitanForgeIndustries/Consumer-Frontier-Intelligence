"""CFI EXP-0009G: copy-anchored full-behavior latent-state replacement.
...
EXP-0009G replaces the top-k-only behavioral objective with full-vocabulary
...
DEFAULT_OUTPUT = data_path("Results", "CFI-Eval-0009E-Copy-Anchored-Full-Behavior")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run CFI EXP-0009G.")
    ...
    p.add_argument("--global-heads", type=int, default=2)
...
class Predictor(nn.Module):
    """Factorized transition predictor with global interaction and local MLP paths.

    The global branch uses compact causal self-attention to approximate
    attention-style token mixing. The local branch is token-wise and approximates
    MLP-style transformation. Both are zero-residual initialized so copy remains
    ...
    def __init__(
        ...
        super().__init__()
        if bottleneck < global_heads or bottleneck % global_heads:
            raise ValueError(
                "bottleneck must be divisible by global_heads and >= global_heads."
            )

        branch_width = bottleneck // 2
        if branch_width < global_heads or branch_width % global_heads:
            raise ValueError(
                "half the bottleneck must be divisible by global_heads."
            )

        self.norm = nn.LayerNorm(hidden_size)

        # Token-wise nonlinear branch, corresponding to the MLP-like part.
        self.local_down = nn.Linear(hidden_size, branch_width)
        self.local_act = nn.GELU()
        self.local_up = nn.Linear(branch_width, hidden_size)

        # Compact causal global interaction branch.
        self.global_q = nn.Linear(hidden_size, branch_width, bias=False)
        self.global_k = nn.Linear(hidden_size, branch_width, bias=False)
        self.global_v = nn.Linear(hidden_size, branch_width, bias=False)
        self.global_out = nn.Linear(branch_width, hidden_size)

        self.global_heads = global_heads
        self.head_dim = branch_width // global_heads
        self.max_update_ratio = max_update_ratio

        # Token-wise gate controls how much of the learned correction comes from
        # each factor. The gate starts neutral at 0.5, while both output paths are
        # zero initialized, preserving exact copy at initialization.
    ...
    def forward(self, source: torch.Tensor) -> torch.Tensor:
        ...
        q = self.global_q(normalized).view(
            batch, sequence, self.global_heads, self.head_dim
        ).transpose(1, 2)
        k = self.global_k(normalized).view(
            batch, sequence, self.global_heads, self.head_dim
        ).transpose(1, 2)
        v = self.global_v(normalized).view(
            batch, sequence, self.global_heads, self.head_dim
        ).transpose(1, 2)
        ...
        global_features = torch.matmul(attention, v)
        global_features = global_features.transpose(1, 2).contiguous().view(
            batch, sequence, -1
        )
        global_branch = self.global_out(global_features)

        gate = torch.softmax(self.gate(normalized), dim=-1)
        update = gate[..., 0:1] * local + gate[..., 1:2] * global_branch
...
def main() -> int:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for EXP-0009G.")
    if args.questions <= 1:
    ...
    if total_layers < 36:
        raise RuntimeError(
            f"EXP-0009G requires at least 36 layers, found {total_layers}."
        )
