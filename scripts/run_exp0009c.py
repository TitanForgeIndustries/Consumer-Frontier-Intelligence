"""CFI EXP-0009C: predictive depth sweep.

Tests whether later transformer hidden states can be predicted directly from
earlier same-position states, and how prediction quality changes with depth gap.
...
The experiment is teacher-forced state substitution. It does not claim a
hardware speedup.
"""
...
DEFAULT_OUTPUT = Path(
    r"E:\Titan Forge Industries\CFI-Data\Results\CFI-Eval-0009C-Predictive-Depth-Sweep"
)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run CFI EXP-0009C.")
    ...
    p.add_argument(
        ...
        type=int,
        default=0,
        help="Maximum evaluated target positions per held-out question; 0 = all.",
    )
    return p.parse_args()
...
def collect_trace(
    ...
    states = {
        24: hidden_states[24][0].float().cpu(),
    ...
    del outputs, hidden_states, layer36_capture, full_ids, output, inputs
...
def state_pairs(
    ...
    if stop <= start:
        empty_source = source.new_empty((0, source.shape[-1]))
        empty_target = target.new_empty((0, target.shape[-1]))
        return empty_source, empty_target

    positions = torch.arange(start, stop)
    return source[positions], target[positions]
...
def train_predictor(
    ...
    pairs = [state_pairs(t, source_layer, target_layer) for t in traces]
    xs = [x for x, y in pairs if x.shape[0] > 0]
    ys = [y for x, y in pairs if y.shape[0] > 0]
    if not xs:
    ...
    predictor = Predictor(x.shape[-1], args.bottleneck).to(
        device=device,
        dtype=torch.float32,
    )
    ...
    generator.manual_seed(1000 + source_layer * 100 + target_layer)
    ...
    for epoch in range(args.epochs):
        ...
        for offset in range(0, x.shape[0], args.batch_size):
            ...
            loss = mse + args.hidden_loss_weight * cosine_loss
    ...
    with torch.inference_mode():
        ...
        pred = predictor(x[:sample].to(device)).cpu()
        truth = y[:sample]
        sample_cosine = float(
            F.cosine_similarity(pred, truth, dim=-1).mean().item()
        )
        sample_relative = float(
            (
    ...
    return predictor, {
        ...
        "sample_train_cosine": sample_cosine,
        "sample_train_relative_error": sample_relative,
...
def distribution_metrics(
    ...
    target = target_ids.to(
        device=oracle.device, dtype=torch.long
    ).unsqueeze(-1)
    ...
    kl = (oracle_p * (oracle_logp - candidate_logp)).sum(dim=-1)
    ...
    return {
        ...
        "logit_l2_mean": float(
            torch.linalg.vector_norm(oracle - candidate, dim=-1)
            .mean()
            .item()
        ),
    }
...
def evaluate(
    ...
    source_layer, target_layer = RELATIONS[relation]
    source, target = state_pairs(trace, source_layer, target_layer)
    if source.shape[0] < 1:
        raise RuntimeError(
            f"No evaluation state pairs for {relation} / question {trace.index}."
        )
    ...
    if max_tokens >= count:
        ...
        keep = torch.linspace(
            0, count - 1, steps=max_tokens
        ).round().long()
        keep = torch.unique(keep, sorted=True)

    positions = (trace.prompt_length - 1) + keep
    ...
    predictor.eval()
    with torch.inference_mode():
        predicted = predictor(source_eval.to("cuda:0")).float()
    ...
    if (
        ...
        raise RuntimeError(
            f"Exact-state injection control failed for {relation} / "
    ...
    pred_state_cos = F.cosine_similarity(
        predicted.cpu(), target_eval, dim=-1
    )
    pred_state_rel = (
    ...
    copy_state_cos = F.cosine_similarity(
        source_eval, target_eval, dim=-1
    )
    copy_state_rel = (
        torch.linalg.vector_norm(source_eval - target_eval, dim=-1)
        / torch.linalg.vector_norm(target_eval, dim=-1).clamp_min(1e-8)
    )

    metrics.update(
        {
            "state_cosine_mean": float(pred_state_cos.mean().item()),
            "state_relative_error_mean": float(pred_state_rel.mean().item()),
            "state_mse_mean": float(
                F.mse_loss(predicted.cpu(), target_eval).item()
            ),
    ...
    return {
        ...
        "copy_baseline": {
            **copy_metrics,
            "state_cosine_mean": float(copy_state_cos.mean().item()),
            "state_relative_error_mean": float(copy_state_rel.mean().item()),
            "state_mse_mean": float(
                F.mse_loss(source_eval, target_eval).item()
            ),
        },
    }
...
def main() -> int:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for EXP-0009C.")
    if args.questions <= 1:
    ...
    if total_layers < 36:
        raise RuntimeError(
            f"EXP-0009C requires at least 36 layers, found {total_layers}."
        )
    ...
    for relation in args.relations:
        ...
        print(
            f"\n=== {relation} "
            f"(gap {target_layer - source_layer}) ==="
        )
        ...
        for trace in eval_traces:
            ...
            print(
                ...
                f"KL={copy['kl_oracle_to_candidate_mean']:.4f} "
                f"top1={copy['top1_agreement']:.4f}"
            )
    ...
    summary = {
        "schema_version": 1,
        "experiment": "EXP-0009C",
        "title": "Predictive Depth Sweep",
        "status": "completed",
        "method": "teacher_forced_predictive_depth_state_injection",
        ...
        "interpretation_guidance": [
            ...
            "This experiment isolates same-position depth prediction from temporal state prediction.",
            "Depth gap is varied to test how much transformer computation can be forecast directly.",
            "Source-state copying is retained as the zero-extra-compute baseline.",
            "Downstream distribution preservation is the primary behavioral metric.",
            "The experiment does not establish a hardware speedup.",
            "A relation that remains strong at larger depth gaps is a candidate for sequential compute replacement.",
        ],
    }
