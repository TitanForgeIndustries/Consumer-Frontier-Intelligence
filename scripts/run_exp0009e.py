"""CFI EXP-0009E: copy-anchored full-behavior latent-state replacement.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run CFI EXP-0009E.")
...
class Predictor(nn.Module):
    def __init__(self, hidden_size: int, bottleneck: int, max_update_ratio: float) -> None:
        super().__init__()
...
def main() -> int:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for EXP-0009E.")
    if args.questions <= 1:
    ...
    if total_layers < 36:
        raise RuntimeError(
            f"EXP-0009E requires at least 36 layers, found {total_layers}."
        )
